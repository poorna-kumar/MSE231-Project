...
w2v_curr_df <- filter(w2v_df, year == curr_year, topic != 'neutral' & topic != 'litcomp') %>%
...
ggsave(paste0("./Plots/Proj",curr_year,".pdf"), height = 5, width = 5)
...
ggplot(w2v_curr_df, aes(x = he_she, y = man_woman, colour = topic)) +
  ...
  xlim(min_proj,max_proj) + 
  ylim(min_proj,max_proj) 
ggsave(paste0("./Plots/ProjLitComp",curr_year,".png"), height = 5, width = 5)
